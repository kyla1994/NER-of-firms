...
def prepare_sentence(str_chars, char_to_id, lower=False):
    """
...
def prepare_dataset(sentences, char_to_id, tag_to_id, use_gaze, lower=False, list_prefix= None, label_cnt= None):
    """
    ...
    for i, s in enumerate(sentences):
        ...
        tags = [tag_to_id[w[-1]] if w[-1] in tag_to_id else 0 for w in s]
        if use_gaze:
            data.append({
                'str_chars': str_chars,
                'chars': chars,
