...
from loader import char_mapping, radical_mapping, tag_mapping
...
optparser.add_option(
    "-J", "--jianti", default="1",
    type='int', help="Use jianti char-radical"
)
optparser.add_option(
    "-r", "--radical_dim", default="50",
    type='int', help="Radical embedding dimension"
)
optparser.add_option(
    "-R", "--radical_hidden_dim", default="50",
    type='int', help="Radical LSTM/GRU hidden layer size"
)
optparser.add_option(
    "-B", "--radical_bidirect", default="1",
    type='int', help="Use a bidirectional LSTM for radicals"
)
optparser.add_option(
    "-p", "--pre_emb", default="",
...
parameters['jianti'] = opts.jianti == 1
parameters['char_dim'] = opts.char_dim
parameters['char_hidden_dim'] = opts.char_hidden_dim
parameters['char_bidirect'] = opts.char_bidirect == 1
parameters['radical_dim'] = opts.radical_dim
parameters['radical_hidden_dim'] = opts.radical_hidden_dim
parameters['radical_bidirect'] = opts.radical_bidirect == 1
...
assert parameters['char_dim'] > 0 or parameters['radical_dim'] > 0
...
# Create a dictionary and a mapping for chars / POS tags / tags
dico_radicals, radical_to_id, id_to_radical = radical_mapping(train_sentences, radicals_dict)
...
train_data = prepare_dataset(
    train_sentences, char_to_id, radical_to_id, tag_to_id, use_gaze, radicals_dict, lower, list_prefix= list_prefix, label_cnt= label_cnt
)
dev_data = prepare_dataset(
    dev_sentences, char_to_id, radical_to_id, tag_to_id, use_gaze, radicals_dict,  lower, list_prefix= list_prefix, label_cnt= label_cnt
)
test_data = prepare_dataset(
    test_sentences, char_to_id, radical_to_id, tag_to_id, use_gaze, radicals_dict, lower, list_prefix= list_prefix, label_cnt= label_cnt
)
...
model.save_mappings(id_to_char, id_to_radical, id_to_tag)
...
F1_file = './vec_' + "gaze_" + str(parameters['use_gaze']) + "char" + str(parameters['char_dim']) + "_" + str(parameters['char_hidden_dim']) \
          + "_radical" + str(parameters['radical_dim']) + "_" + str(parameters['radical_hidden_dim']) + '.txt'
...
for epoch in xrange(n_epochs):
    ...
    for i, index in enumerate(np.random.permutation(len(train_data))):
        count += 1
        input = create_input(train_data[index], parameters, True, use_gaze, singletons)
